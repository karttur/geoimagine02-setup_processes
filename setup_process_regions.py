...
from postgresdb import ManageProcess, SelectUser, ManageMODIS, ManageRegion, ManageAncillary, ManageSentinel, ManageLandsat
...
from params import Params, Struct

from base64 import b64encode

import netrc

from setup_processes import PGsession, ProcessDefaultRegions
...
def DegreeTiles(pgdb):
    '''Setup default region tiles covering 1deg by 1deg
    '''
    
    session = SelectUser(pgdb)
    ...
    session = ManageRegion(pgdb)

    #session._LoadBulkDefregions(tmpFPN)
    session._LoadBulkRegions(regFPN, False)
...
def SetupOldest(relpath,projFN,verbose):
    '''
    Setup processes
    '''
...
def SetupOld(docpath, projFN, db, verbose):
    '''
    Setup processes
    '''
...
if __name__ == "__main__":
    ...
    if MODIS:
        ...
        Setup('modisdoc', projFN, pgdb, verbose)
